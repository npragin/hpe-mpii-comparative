    ...
    def evaluate(self,
        ...
        for metric in metrics:
            ...
            if metric == 'proposal':
                ...
                with contextlib.redirect_stdout(redirect_string):
                    cocoEval.summarize()
                # Add OKS to output if using keypoints
                if metric == 'keypoints' and "oks" in cocoEval.eval:
                    print_log(f'\nAverage OKS: {cocoEval.eval["oks"]:.3f}', logger=logger)
                ...
                # Add OKS to results if using keypoints
                if metric == 'keypoints' and "oks" in cocoEval.eval:
                    key = f'{metric}_OKS'
                    val = float(f'{cocoEval.eval["oks"]:.3f}')
                    eval_results[key] = val
                ap = cocoEval.stats[:5]
                eval_results[f'{metric}_mAP_copypaste'] = (
                    f'{ap[0]:.3f} {ap[1]:.3f} {ap[2]:.3f} {ap[3]:.3f} '
                    f'{ap[4]:.3f} {cocoEval.eval["oks"]:.3f}')
